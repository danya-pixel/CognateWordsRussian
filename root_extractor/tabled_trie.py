...
class Trie:
    ...
    def __init__(
        ...
        alphabet,
        make_sorted=True,
        make_alphabet_codes=True,
        is_numpied=False,
        to_make_cashed=False,
        precompute_symbols=None,
        allow_spaces=False,
        dict_storage=False,
    ):
        self.alphabet = sorted(alphabet) if make_sorted else alphabet
        self.alphabet_codes = (
            {a: i for i, a in enumerate(self.alphabet)}
            if make_alphabet_codes
            else self.alphabet
        )
    ...
    def _make_default_node(self):
        if self.dict_storage:
            ...
            return np.full(
                shape=(len(self.alphabet),), fill_value=Trie.NO_NODE, dtype=int
            )
        else:
        ...
        with open(outfile, "w", encoding="utf8") as fout:
            ...
            for index, label in enumerate(self.final):
                ...
                fout.write(
                    "{}\t{}\n".format(
                        "T" if label else "F",
                        " ".join(
                            "{}:{}".format(*elem) for elem in zip(letters, children)
                        ),
                    )
                )
            if self.precompute_symbols is not None:
                for elem in self.data:
                    fout.write(
                        ":".join(",".join(map(str, symbols)) for symbols in elem) + "\n"
                    )
        return
    ...
    def find_partitions(self, s, max_count=1):
        ...
        for curr, borders, cost in curr_agenda:
            if curr == self.root:
                borders = [0] + borders
                answer.append(
                    [s[left : borders[i + 1]] for i, left in enumerate(borders[:-1])]
                )
        return answer
    ...
    def descend_by_prefixes(
        self, s, max_count=1, start_pos=0, start_node=None, return_pairs=False
    ):
            ...
            for curr, starts_with_ranks in curr_agenda.items():
                ...
                for start, rank in starts_with_ranks.items():
                    ...
                    if is_final:
                        ...
                        if rank < max_count:
                            if (
                                i + 1 not in new_agenda[self.root]
                                or rank + 1 < new_agenda[self.root][i + 1]
                            ):
    ...
    def __repr__(self):
        ...
        for i, (final, data) in enumerate(zip(self.final, self.data)):
            ...
            if data is not None:
                answer += "data:{0} {1}\n".format(
                    len(data), " ".join(str(elem) for elem in data)
                )
        return answer
    ...
    def _get_letters(self, index, return_indexes=False):
        ...
        if self.dict_storage:
            ...
            answer = [
                i for i, elem in enumerate(self.graph[index]) if elem != Trie.NO_NODE
            ]
    ...
    def _get_children_and_letters(self, index, return_indexes=False):
        if self.dict_storage:
            ...
            answer = [
                elem for elem in enumerate(self.graph[index]) if elem[1] != Trie.NO_NODE
            ]
        if not return_indexes:
...
class TrieMinimizer:
    ...
    def minimize(
        ...
        if dict_storage:
            ...
            new_graph = np.full(
                shape=(L, len(trie.alphabet)), fill_value=Trie.NO_NODE, dtype=int
            )
            new_final = np.array(new_final, dtype=bool)
        else:
            new_graph = [[Trie.NO_NODE for a in trie.alphabet] for i in range(L)]
        for (indexes, children, final), class_index in sorted(
            classes.items(), key=(lambda x: x[1])
        ):
...
def load_trie(infile):
    with open(infile, "r", encoding="utf8") as fin:
        ...
        if trie.dict_storage:
            ...
            graph = np.full(
                shape=(nodes_number, len(alphabet)), fill_value=Trie.NO_NODE, dtype=int
            )
        else:
...
def make_trie(
    ...
    trie = Trie(
        alphabet,
        is_numpied=is_numpied,
        to_make_cashed=make_cashed,
        precompute_symbols=precompute_symbols,
        dict_storage=dict_storage,
    )
    trie.fit(words)
...
def test_precomputing_symbols():
    ...
    trie = Trie(alphabet, is_numpied=False, precompute_symbols=2)
    trie.fit(words[:10])
    compressed, node_classes = tm.minimize(
        trie, precompute_symbols=2, return_groups=True
    )
    possible_continuations = [set() for _ in compressed.graph]
    for future_symbols, index in zip(trie.data, node_classes):
        possible_continuations[index].add(
