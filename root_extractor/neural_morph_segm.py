...
from root_extractor.tabled_trie import make_trie
...
def collect_buckets(lengths, buckets_number, max_bucket_size=-1):
    ...
    if max_bucket_size != -1:
        ...
        indexes = [
            curr_indexes[start : start + max_bucket_size]
...
def get_next_morpheme(morpheme):
    ...
    answer = [
        "{}-{}".format(x, y) for x in new_morpheme_labels for y in new_morpheme_types
    ]
    return answer
...
def is_correct_morpheme_sequence(morphemes):
    ...
    if morpheme_label not in "ES" or morpheme_type not in [
        "ROOT",
        "SUFF",
        "ENDING",
        "POSTFIX",
...
class Partitioner:
    ...
    def __init__(
        self,
        models_number=1,
        use_morpheme_types=True,
        to_memorize_morphemes=False,
        min_morpheme_count=2,
    ...
    def check_params(self):
        ...
        if self.filters_number.shape[0] == 1:
            self.filters_number = np.repeat(
                self.filters_number, len(self.window_size), axis=0
            )
        if self.filters_number.shape[0] != len(self.window_size):
            raise ValueError(
                "Filters array should have shape (len(window_size), conv_layers)"
            )
        if self.filters_number.shape[1] == 1:
            self.filters_number = np.repeat(
                self.filters_number, self.conv_layers, axis=1
            )
        ...
        if self.callbacks is None:
            self.callbacks = []
        if self.early_stopping is not None and not any(
            isinstance(x, EarlyStopping) for x in self.callbacks
        ):
            self.callbacks.append(
                EarlyStopping(patience=self.early_stopping, monitor="accuracy")
            )
    ...
    def to_json(self, outfile, model_file=None):
        ...
        model_files = [
            make_model_file(model_file, i + 1) for i in range(self.models_number)
        ]
        ...
        for (attr, val) in inspect.getmembers(self):
            ...
            if not (
                ...
                or isinstance(getattr(Partitioner, attr, None), property)
                or attr.isupper()
                or attr
                in [
                    "callbacks",
    ...
    def _preprocess(self, data, targets=None):
        ...
        data_by_buckets = [
            self._make_bucket_data(data, length, indexes)
            for length, indexes in buckets_with_indexes
        ]
        # targets=None --- предсказание, иначе --- обучение
        if targets is not None:
            targets_by_buckets = [
                self._make_bucket_data(targets, length, indexes, is_target=True)
                for length, indexes in buckets_with_indexes
            ]
    ...
    def _make_bucket_data(self, data, bucket_length, bucket_indexes, is_target=False):
        ...
        if is_target:
            return self._recode_bucket_data(
                bucket_data, bucket_length, self.target_symbol_codes_
            )
        else:
            answer = [
    ...
    def _make_morpheme_data(self, data, bucket_length):
        ...
        for j, word in enumerate(data):
            ...
            postfix_lengths = self.right_morphemes_["postfix"].descend_by_prefixes(
                word[:0:-1]
            )
            ...
            for end in ending_ends[::-1]:
                ending_lengths = self.right_morphemes_["end"].descend_by_prefixes(
                    word[end - 1 : 0 : -1]
                )
                for k in ending_lengths:
                    ...
                    if k == 1:
                        ...
                        curr_answer[end - k + 1, 3] = max(
                            score, curr_answer[end - k + 1, 3]
                        )
            ...
            suffixes = self.right_morphemes_["suff"].descend_by_prefixes(
                word[::-1],
                start_pos=[m - k for k in suffix_ends],
                max_count=3,
                return_pairs=True,
            )
            ...
            for first, last in suffixes:
                ...
                if last == first + 1:
                    ...
                    curr_answer[m - last + 1, 2] = max(
                        score, curr_answer[m - last + 1, 2]
                    )
                    curr_answer[m - first, 7] = max(score, curr_answer[m - first, 7])
                suffix_starts.add(m - last)
            for start in root_starts:
                root_ends = self.left_morphemes_["root"].descend_by_prefixes(
                    word[start:]
                )
                for end in root_ends:
                    score = self._get_ngram_score(word[start:end], "root")
                    if end == start + 1:
                        curr_answer[start + 1, 11] = max(
                            score, curr_answer[start + 1, 11]
                        )
                    else:
                        curr_answer[start + 1, 1] = max(
                            score, curr_answer[start + 1, 1]
                        )
                        curr_answer[end, 6] = max(score, curr_answer[end, 6])
            for end in suffix_starts:
                root_lengths = self.right_morphemes_["root"].descend_by_prefixes(
                    word[end - 1 : -1 : -1]
                )
                for k in root_lengths:
                    ...
                    if k == 1:
                        ...
                        curr_answer[end - k + 1, 1] = max(
                            curr_answer[end - k + 1, 1], score
                        )
        ...
        for j, word in enumerate(data):
            ...
            for starts, end in positions:
                for start in starts:
                    ...
                    if end == start + 1:
                        curr_answer[start + 1, 2] = max(
                            curr_answer[start + 1, 2], score
                        )
                    else:
                        curr_answer[start + 1, 0] = max(
                            curr_answer[start + 0, 2], score
                        )
    ...
    def train(self, source, targets, dev=None, dev_targets=None, model_file=None):
        ...
        if dev is not None:
            dev_data_by_buckets, dev_targets_by_buckets, _ = self._preprocess(
                dev, dev_targets
            )
        else:
    ...
    def build_model(self):
        ...
        if self.use_embeddings:
            symbol_embeddings = kl.Embedding(
                self.symbols_number_, self.embeddings_size, name="symbol_embeddings"
            )(symbol_inputs)
        else:
        ...
        if self.to_memorize_morphemes:
            ...
            context_inputs = kl.Input(
                shape=(None, self.memory_dim), dtype="float32", name="context_inputs"
            )
        ...
        conv_outputs = []
        for window_size, curr_filters_numbers in zip(
            self.window_size, self.filters_number
        ):
            ...
            for j, filters_number in enumerate(curr_filters_numbers[:-1]):
                ...
                curr_conv_input = kl.Conv1D(
                    filters_number, window_size, activation="relu", padding="same"
                )(curr_conv_input)
                if self.dropout > 0.0:
                    # между однотипными слоями рекомендуется вставить dropout
                    curr_conv_input = kl.Dropout(self.dropout)(curr_conv_input)
            if not self.use_lstm:
                curr_conv_output = kl.Conv1D(
                    curr_filters_numbers[-1],
                    window_size,
                    activation="relu",
                    padding="same",
                )(curr_conv_input)
        ...
        if self.use_lstm:
            conv_output = kl.Bidirectional(
                kl.LSTM(self.lstm_units, return_sequences=True)
            )(conv_output)
        if self.dense_output_units:
            pre_last_output = kl.TimeDistributed(
                kl.Dense(self.dense_output_units, activation="relu"), name="pre_output"
            )(conv_output)
        else:
        ...
        output = kl.TimeDistributed(
            kl.Dense(self.target_symbols_number_, activation="softmax"), name="output"
        )(pre_last_output)
    ...
    def _train_models(
        ...
        data_by_buckets,
        targets_by_buckets,
        dev_data_by_buckets=None,
        dev_targets_by_buckets=None,
        model_file=None,
    ):
        """
        ...
        if dev_data_by_buckets is not None:
            train_indexes_by_buckets = [
                list(range(len(bucket[0]))) for bucket in data_by_buckets
            ]
            for elem in train_indexes_by_buckets:
                np.random.shuffle(elem)
            dev_indexes_by_buckets = [
                list(range(len(bucket[0]))) for bucket in dev_data_by_buckets
            ]
            ...
            for bucket in data_by_buckets:
                ...
                train_indexes_by_buckets.append(
                    indexes_for_bucket[:train_bucket_length]
                )
        ...
        train_batches_indexes = list(
            chain.from_iterable(
                [
                    [
                        (i, elem[j : j + self.batch_size])
                        for j in range(0, len(elem), self.batch_size)
                    ]
                    for i, elem in enumerate(train_indexes_by_buckets)
                ]
            )
        )
        dev_batches_indexes = list(
            chain.from_iterable(
                [
                    [
                        (i, elem[j : j + self.batch_size])
    ...
    def _memorize_morphemes(self, words, targets):
        ...
        for key, counts in morphemes.items():
            self.morphemes_[key] = [
                x for x, count in counts.items() if count >= self.min_morpheme_count
            ]
    ...
    def _memorize_ngram_counts(self, words, counts):
        ...
        """
        prefix_counts, suffix_counts, ngram_counts = (
            defaultdict(int),
            defaultdict(int),
            defaultdict(int),
        )
    ...
    def _make_morpheme_tries(self):
        ...
        if self.use_morpheme_types:
            for key in self.LEFT_MORPHEME_TYPES:
                self.left_morphemes_[key] = make_trie(
                    list(self.morphemes_.get(key, ""))
                )
            for key in self.RIGHT_MORPHEME_TYPES:
                self.right_morphemes_[key] = make_trie(
                    [x[::-1] for x in self.morphemes_.get(key, "")]
                )
    ...
    def labels_to_morphemes(
        self, word, labels, probs=None, return_probs=False, return_types=False
    ):
        """
        ...
        if return_probs:
            if probs is None:
                Warning(
                    "Для вычисления вероятностей морфем нужно передать вероятности меток"
                )
                return_probs = False
        if return_probs:
    ...
    def predict(self, words, return_probs=False):
        ...
        return [
            self.labels_to_morphemes(word, elem[0], elem[1], return_probs=return_probs)
    ...
    def _decode_best(self, probs, length):
        ...
        if not is_correct_morpheme_sequence(best_labels[1:]):
            ...
            initial_costs[BEGIN], initial_states[BEGIN] = (
                -np.log(probs[0, BEGIN]),
                BEGIN,
            )
            ...
            possible_states = [
                self.target_symbol_codes_["{}-{}".format(x, y)]
                for x in "ES"
                for y in ["ROOT", "SUFF", "END", "POSTFIX", "None"]
        ...
        probs_to_return = np.zeros(
            shape=(length, self.target_symbols_number_), dtype=np.float32
        )
    ...
    def get_possible_next_states(self, state_index):
        ...
        return [
            self.target_symbol_codes_[x]
            for x in next_states
            if x in self.target_symbol_codes_
        ]
...
def measure_quality(
    targets, predicted_targets, english_metrics=False, measure_last=True
):
    """
...
if __name__ == "__main__":
    ...
    if "test_file" in params:
        ...
        quality = measure_quality(
            targets,
            [elem[0] for elem in predicted_targets],
                ...
                for word, (labels, probs) in zip(inputs, predicted_targets):
                    ...
                    fout.write(
                        format_string.format(
                            word,
                            "/".join(
                                morph_format_string.format(*elem)
