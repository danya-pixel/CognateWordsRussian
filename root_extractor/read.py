...
def read_BMES(
    infile, transform_to_BMES=True, n=None, morph_sep="/", sep=":", shuffle=True
):
    source, targets = [], []
    with open(infile, "r", encoding="utf8") as fin:
        for line in fin:
            ...
            morphs, morph_types = [elem[0] for elem in analysis], [
                elem[1] for elem in analysis
            ]
...
def read_input(infile, transform_to_BMES=True, n=None, shuffle=True):
    ...
    with open(infile, "r", encoding="utf8") as fin:
        for line in fin:
            ...
            target = (
                partition_to_BMES(morphs, analysis) if transform_to_BMES else morphs
            )
            source.append(word)
            targets.append(target)
    if n is not None:
